...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		List<Car> candidates = CarShopStorage.getInstancce().getCars().get(car.getType());
		if (candidates != null && candidates.contains(car)) {
			candidates.remove(car);
		} else {
			throw new RequestedCarNotFoundException("Car not found in database");
		}
	}
	...
	public void addCarToStorage(Car car) {
		if (car.getPrice() <= 0 ) {
			throw new IllegalArgumentException("Price of the car can not be negative");
		}
